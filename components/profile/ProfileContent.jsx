...
export default function ProfileContent({
  ...
  const category = useMemo(() => {
    return categories.find(
      (item) =>
        item.id === currentProfile.categoryId,
    );
  }, [
    categories,
  ...
  function handleWorksUpdated(
    updatedWorks,
  ) {
    setCurrentWorks(
      Array.isArray(updatedWorks)
        ? updatedWorks
        : [],
    );
  }
          ...
          <aside className="space-y-6">
            ...
            <section className="rounded-3xl border border-slate-200 bg-slate-950 p-5 text-white">
              ...
              <p className="mt-2 text-xs leading-5 text-white/60">
                Add your skills, services and
                best works so people can
            ...
            <section className="rounded-3xl border border-red-100 bg-white p-5 shadow-sm">
              ...
              <p className="mt-2 text-[11px] leading-5 text-slate-400">
                Deleting your profile permanently
                removes your Youth Space profile.
              </p>
...
function WorkCard({
  ...
  const category = categories.find(
    (item) =>
      item.id === work.categoryId ||
      item.id === work.category,
  );
